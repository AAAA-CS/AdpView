...
class PretrainViewMakerSystem(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # if config.data_params.dataset != 'potsdam' and config.data_params.dataset != 'loveDa':
    ...
    def create_viewmaker(self):
        view_model = viewmaker.Viewmaker(
            ...
            distortion_budget=self.config.model_params.view_bound_magnitude,        #0.05
            activation=self.config.model_params.generator_activation or 'relu',     #relu
            clamp=self.config.model_params.clamp_views,                             #none
            frequency_domain=self.config.model_params.spectral or False,            #False
            downsample_to=self.config.model_params.viewmaker_downsample or False,   #False
            num_res_blocks=self.config.model_params.num_res_blocks or 5,            #3
        )
        return view_model
    ...
    def training_step_end(self, emb_dict):
        ...
        # print("viewmaker_loss", view_maker_loss)
...
class TransferExpertSystem(TransferViewMakerSystem):
    ...
    def load_pretrained_model(self):
        ...
        config_path = os.path.join(base_dir, 'config.json')        #base_dir-->str(base_dir)
        config_json = utils.load_json(config_path)
        config = DotMap(config_json)
